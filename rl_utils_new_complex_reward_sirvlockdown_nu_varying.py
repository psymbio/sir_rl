...
class SIREnvironment(gym.Env):
    ...
    def render(self, score=0.0, stringency=np.array([]), learning=False):
        ...
        # modelling_type = "with_lockdown_with_vaccination_time_varying_nu"
        # axes[0, 0].plot(self.df['date'], self.df['S']/self.df['N'], 'b', alpha=0.5, lw=2, label='Susceptible (actual)')
        # axes[0, 0].plot(self.df['date'], self.df['I']/self.df['N'], 'r', alpha=0.5, lw=2, label='Infected (actual)')
        # axes[0, 0].plot(self.df['date'], self.df['R']/self.df['N'], 'g', alpha=0.5, lw=2, label='Recovered (actual)')
        # axes[0, 0].plot(self.df['date'], self.df['S_modelled_' + modelling_type]/self.N, 'b:', alpha=0.5, lw=2, label='Susceptible (modelled)')
        # axes[0, 0].plot(self.df['date'], self.df['I_modelled_' + modelling_type]/self.N, 'r:', alpha=0.5, lw=2, label='Infected (modelled)')
        # axes[0, 0].plot(self.df['date'], self.df['R_modelled_' + modelling_type]/self.N, 'g:', alpha=0.5, lw=2, label='Recovered (modelled)')
        # axes[0, 0].plot(self.df['date'], self.df['S_moves']/self.N, 'b--', alpha=0.5, lw=2, label='Susceptible (rl)')
        # axes[0, 0].plot(self.df['date'], self.df['I_moves']/self.N, 'r--', alpha=0.5, lw=2, label='Infected (rl)')
        # axes[0, 0].plot(self.df['date'], self.df['R_moves']/self.N, 'g--', alpha=0.5, lw=2, label='Recovered (rl)')
        ...
        # axes[0, 1].plot(self.df['date'], self.df['I']/self.df['N'], 'r', alpha=0.5, lw=2, label='Infected (actual)')
        # axes[0, 1].plot(self.df['date'], self.df['I_modelled_' + modelling_type]/self.N, 'r:', alpha=0.5, lw=2, label='Infected (modelled)')
        # axes[0, 1].plot(self.df['date'], self.df['I_moves']/self.N, 'r--', alpha=0.5, lw=2, label='Infected (rl)')
        # axes[0, 1].set_xlabel("Date")
        # axes[0, 1].set_ylabel("Percentage of Infected Population")
        # axes[0, 1].set_title("SIR Epidemic Trajectory (Infected)")
        # axes[0, 1].grid(True)
        # axes[0, 1].legend()

        # axes[1, 0].plot(self.df['date'], self.df['stringency_index'], 'b', label="Stringency (actual)")
        # axes[1, 0].plot(self.df['date'], self.store_stringency, 'g', label="Stringency (rl)")
        # axes[1, 0].set_xlabel("Date")
        # axes[1, 0].set_ylabel("Stringency Index")
        # axes[1, 0].set_title("Stringency over Time")
        # axes[1, 0].set_ylim(bottom=0, top=150)
        # axes[1, 0].grid(True)
        # axes[1, 0].legend()

        # axes[1, 1].plot(self.df['date'], self.df['gdp_normalized'], 'r', label="GDP normalized (actual)")
        # axes[1, 1].plot(self.df['date'], self.df['gdp_normalized_modelled'], 'b', label="GDP normalized (modelled)")
        # axes[1, 1].plot(self.df['date'], self.df['gdp_normalized_moves'], 'g', label="GDP normalized (rl)")
        # axes[1, 1].set_xlabel("Date")
        # axes[1, 1].set_ylabel("GDP")
        # axes[1, 1].set_title("GDP over Time")
        # axes[1, 1].set_ylim(bottom=0, top=150)
        # axes[1, 1].grid(True)
        # axes[1, 1].legend()
        ...
        # axes[2, 0].plot(self.df['date'], self.df['r_eff_actual_' + modelling_type], 'r', label="R_eff (actual)")
        # axes[2, 0].plot(self.df['date'], self.df['r_eff_modelled_' + modelling_type], 'b', label="R_eff (modelled)")
        # axes[2, 0].plot(self.df['date'], self.df["r_eff_moves_" + modelling_type], 'g', label="R_eff (rl)")
        # axes[2, 0].set_xlabel("Date")
        # axes[2, 0].set_ylabel("R_eff")
        # axes[2, 0].set_title("R_eff over Time")
        # axes[2, 0].set_ylim(bottom=0, top=7.0)
        # axes[2, 0].grid(True)
        ...
        # axes[2, 1].plot(self.df['date'], reward_actual, 'r', label="Reward (actual)")
        # axes[2, 1].plot(self.df['date'], reward_modelled, 'b', label="Reward (modelled)")
        # axes[2, 1].plot(self.df['date'], reward_moves, 'g', label="Reward (rl)")
        # axes[2, 1].set_xlabel("Time /days")
        # axes[2, 1].set_ylabel("Reward")
        # axes[2, 1].set_title("Reward over Time")
        # axes[2, 1].grid(True)
        # axes[2, 1].legend()
        ...
        # formatted_rl_score = "{:.2f}".format(reward_moves.sum())
        # # formatted_rl_score = "{:.2f}".format(score)
        # # formatted_rl_score_2 = "{:.2f}".format(self.store_reward.sum())

        # axes[3, 0].text(0.5, 0.5, f"Episode score (actual): {formatted_actual_score}\nEpisode score (modelled): {formatted_modelled_score}\nEpisode Score (rl): {formatted_rl_score}", ha='center', va='center', transform=axes[2, 1].transAxes, fontsize=14)
        # axes[3, 0].set_xticks([])
        # axes[3, 0].set_yticks([])
