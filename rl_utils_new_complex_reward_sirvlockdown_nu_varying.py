...
class SIREnvironment(gym.Env):
    ...
    def render(self, score=0.0, learning=False):
        ...
        axes[0, 0].plot(self.t, self.df['S_modelled_' + modelling_type]/self.N, 'b:', alpha=0.5, lw=2, label='Susceptible (modelled)')
        axes[0, 0].plot(self.t, self.df['I_modelled_' + modelling_type]/self.N, 'r:', alpha=0.5, lw=2, label='Infected (modelled)')
        axes[0, 0].plot(self.t, self.df['R_modelled_' + modelling_type]/self.N, 'g:', alpha=0.5, lw=2, label='Recovered (modelled)')
        ...
        axes[1, 1].plot(self.t, self.df['r_eff_actual_' + modelling_type], 'r', label="R_eff (actual)")
        axes[1, 1].plot(self.t, self.df['r_eff_modelled_' + modelling_type], 'b', label="R_eff (modelled)")
        axes[1, 1].plot(self.t, self.store_r_eff, 'g', label="R_eff (rl)")
        first_time_r_eff_actual_1 = next((t for t, r_eff in zip(self.t, self.df['r_eff_actual_' + modelling_type]) if r_eff <= 1), None)
        first_time_r_eff_modelled_1 = next((t for t, r_eff in zip(self.t, self.df['r_eff_modelled_' + modelling_type]) if r_eff <= 1), None)
